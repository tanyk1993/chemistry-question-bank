...
FIG_CAPTION_RE = re.compile(
    r"^((?:%s\s*)+)((?:<b>\s*)?(?:Fig\.|Table)\s*\d+\.\d+(?:\s*</b>)?)\s*$"
...
def parse(document_xml, rels_xml, numbering_xml=None):
    ...
    # merging: `_merge_owner` decides what is one picture by looking at the
